...
def _single_worker_inference(model_creator,
    ...
    # Read data
    assert hparams.infer_batch_size == 1
    (infer_graph, infer_model, infer_src_placeholder,
     infer_batch_size_placeholder, infer_iterator) = create_infer_model(model_creator, hparams, scope)
    with tf.Session(graph=infer_graph, config=utils.get_config_proto()) as sess:
        model_helper.create_or_load_model(infer_model, model_dir, sess, hparams.out_dir, "infer")
        while True:
            input_sentence = raw_input("\nsentence: ")
            sess.run(
                infer_iterator.initializer,
                feed_dict={
                    infer_src_placeholder: [input_sentence],
